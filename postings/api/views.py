...
from .serializers import BlogPostSerializer, LRAQWorkerSerializer
...
class LRAQWorkerAPIView(mixins.CreateModelMixin, generics.ListAPIView): # DetailView CreateView FormView
    lookup_field            = 'surveyId' # slug, id # url(r'?P<pk>\d+')
    serializer_class        = LRAQWorkerSerializer
    #queryset                = BlogPost.objects.all()

    def get_queryset(self):
        qs = LRAQWorker.objects.all()
        query = self.request.GET.get("q")
        if query is not None:
            qs = qs.filter(
                    Q(title__icontains=query)|
